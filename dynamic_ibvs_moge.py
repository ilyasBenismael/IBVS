...
def set_cameras(cur_pose_vect) :
    ...
    des_camera_axis = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.5)
    cur_camera_axis = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.4)
...
def main():
    ...
    cur_pose_vect = [-0.5, 0.4, 0.5, -np.pi/15, -np.pi/20, np.pi/12]  
    ...
    try:    
        for i in range(500) :
            ...
            if(i == 0) :
                plot_img(cur_img, "initial image")
            #plot_img(cur_depth_map, "curr_depthMap")

            # Get current img features and their top 4 matches with desired img
            ...
            cur_feats, des_feats = get_matches(nbr_features, cur_kp, cur_des, des_kp, des_desc, cur_img, des_img, show)

            # Get Ss Ss* and Ss_depths
            Ss = get_Ss_from_uv(cur_feats)
            Ss_star = get_Ss_from_uv(des_feats)
            Ss_Z = get_feats_depth(cur_feats, cur_depth_map)
    ...
    # for static
    # in first iteration after getting the uvs of matches, i wil find the corresponding 3d points in wf (ofc gett ss*)
    # _______then in loop : 
    # express these 4 points in new cur_cam_frame get their zs nd get the ss directly, (print error), get intr matrix and V and get new cam homog


    # Gaussian :
    # init gaussians onces in dffrnt ways see how the render is, in first iteration render from gaussians des n
